...
// Constants
// ユーザー検索
const USER_SEARCH_LIMIT_DEFAULT = 20;
const USER_SEARCH_LIMIT_MAX = 50;
const PAGE_DEFAULT = 1;

// ユーザーの投稿一覧
const USER_POSTS_LIST_LIMIT_DEFAULT = 20;
const USER_POSTS_LIST_LIMIT_MAX = 50;

export const usersRoute = new Hono<Env>()
  .get('/search', requireAuth, async (c) => {
    const currentUser = c.get('user');
    const rawQuery = c.req.query('q') ?? '';
    const rawPage = c.req.query('page') ?? PAGE_DEFAULT;
    const rawLimit = c.req.query('limit') ?? USER_SEARCH_LIMIT_DEFAULT;
    ...
    const limit = Math.min(USER_SEARCH_LIMIT_MAX, Math.max(1, limitNumber));
  ...
  .get('/:handle/posts', requireAuth, async (c) => {
    ...
    const rawPage = c.req.query('page') ?? PAGE_DEFAULT;
    const rawLimit = c.req.query('limit') ?? USER_POSTS_LIST_LIMIT_DEFAULT;
    const pageNumber = rawPage === '' ? NaN : Number(rawPage);
    const limitNumber = rawLimit === '' ? NaN : Number(rawLimit);
    if (Number.isNaN(pageNumber) || Number.isNaN(limitNumber)) {
      return c.json({ error: 'invalid page/limit' }, 400);
    ...
    const page = Math.max(1, pageNumber);
    const limit = Math.min(USER_POSTS_LIST_LIMIT_MAX, Math.max(1, limitNumber));
